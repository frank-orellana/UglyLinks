...
const test_db_name : string = 'test-uglylinks-db';
...
describe('Uglylinks', () => {
  ...
  describe('Tables created', () => {
    it('Tables created', async () => {
      const db : DB | undefined = ulApp.links.storage.db;
      chai.expect(db?db.objectStoreNames.contains('uglyfied_links') : false).equal(true);
    });
  });

  describe('Insertion test', () => {
    const testURLs : string[] = ['test1.test','test2.test','test3.test','test4.test']
  ...
  describe('Removal test', () => {
    it('Links removed', async () => {
      ...
      const arr : Array<LinkProps> = await ulApp.links.getLinksArray() || [];
      
      chai.expect(await ulApp.toggleURL(arr[0].url)).eq(false);

      chai.expect(await ulApp.links.size, 
        'Size of links should be reduced after removing one url')
        .eq(currentCount - 1);
