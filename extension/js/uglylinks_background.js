...
async function menuHandler(info, _tab) {
...
ulApp.init().then(() => {
    browser.runtime.onMessage.addListener(async (msg) => {
        ...
        switch (msg.type) {
            case "get-links":
                let links = await ulApp.links.getLinksArray() || [];
                const links2 = links.map((v) => v.url);
                return new Promise((resolve) => {
                    resolve({ type: "links", links: links2 });
                });
            case "get-disabled-webs":
                let disabledWebs = await ulApp.disabledWebsites.getLinksArray() || [];
                const dwebs = disabledWebs.map((v) => v.url);
                return new Promise((resolve) => {
                    resolve({ type: "disabledWebsites", disabledWebs: dwebs });
                });
            case "is-this-url-disabled":
                let disabled = await ulApp.disabledWebsites.isUrlDisabled(msg.url);
                return new Promise((resolve) => {
